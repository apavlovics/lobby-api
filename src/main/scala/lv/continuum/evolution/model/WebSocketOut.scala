...
@ConfiguredJsonCodec
case class LoginOut($type: String, userType: String) extends WebSocketOut

@ConfiguredJsonCodec
case class PingOut($type: String = "pong", seq: Long) extends WebSocketOut

@ConfiguredJsonCodec
case class RemoveTableOut($type: String = "table_removed", id: Long) extends WebSocketOut with PushNotificationOut

@ConfiguredJsonCodec
case class TableListOut($type: String = "table_list", tables: Seq[Table]) extends WebSocketOut

@ConfiguredJsonCodec
case class ErrorTableOut($type: String, id: Long) extends WebSocketOut

@ConfiguredJsonCodec
case class ErrorOut($type: String = "unknown_error") extends WebSocketOut

@ConfiguredJsonCodec
case class Table(id: Long, name: String, participants: Long)
